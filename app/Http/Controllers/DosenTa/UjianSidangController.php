...
class UjianSidangController extends Controller
{
    ...
    public function cetak_lembar_pengesahan($id) {
        ...
        if (!$pembimbing[0]['ttd'] || !file_exists(public_path('dist/img/' . $pembimbing[0]['ttd']))) {
            return redirect()->route('ujian-sidang.index')->with('error', 'File tanda tangan tidak ditemukan untuk dosen: ' . $pembimbing[0]['nama']);
        }

        if (!$pembimbing[1]['ttd'] || !file_exists(public_path('dist/img/' . $pembimbing[1]['ttd']))) {
            return redirect()->route('ujian-sidang.index')->with('error', 'File tanda tangan tidak ditemukan untuk dosen: ' . $pembimbing[1]['nama']);
        }

        if (!$infoujian->penguji1_ttd_path || !file_exists(public_path('dist/img/' . $infoujian->penguji1_ttd_path))) {
            return redirect()->route('ujian-sidang.index')->with('error', 'File tanda tangan tidak ditemukan untuk dosen: ' . $infoujian->penguji1_nama);
        }

        if (!$infoujian->penguji2_ttd_path || !file_exists(public_path('dist/img/' . $infoujian->penguji2_ttd_path))) {
            return redirect()->route('ujian-sidang.index')->with('error', 'File tanda tangan tidak ditemukan untuk dosen: ' . $infoujian->penguji2_nama);
        }

        if (!$infoujian->penguji3_ttd_path || !file_exists(public_path('dist/img/' . $infoujian->penguji3_ttd_path))) {
            return redirect()->route('ujian-sidang.index')->with('error', 'File tanda tangan tidak ditemukan untuk dosen: ' . $infoujian->penguji3_nama);
        }

        if (!$noSk->file_ttd || !file_exists(public_path('dist/img/' . $noSk->file_ttd))) {
            return redirect()->route('ujian-sidang.index')->with('error', 'File tanda tangan tidak ditemukan untuk dosen: ' . $noSk->nama_kajur);
        }
